...
async function initDb() {
  ...
  await runQuery(`CREATE TABLE IF NOT EXISTS aimessages (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_role TEXT,
...
function buildSearch(filters) {
  ...
  if (filters.q) {
    ...
    for (let i = 0; i < 7; i++) params.push(`%${filters.q}%`);
  }
...
app.get('/jobs', async (req, res) => {
  ...
  res.render('jobs', {
    currentPage: 'jobs',
    title: 'Jobs',
    subtitle: 'Explore jobs in the north of Israel.',
    items
  });
});
...
app.get('/internships', async (req, res) => {
  ...
  res.render('internships', {
    ...
    title: 'Internships',
    subtitle: 'Explore internships for students and graduates.',
    items
  });
});
...
app.get('/projects', async (req, res) => {
  ...
  res.render('projects', {
    ...
    title: 'Projects',
    subtitle: 'Explore applied projects and innovation initiatives.',
    items
  });
});
...
app.post('/manage/opportunity/new', requireRole('admin', 'employer'), async (req, res) => {
  ...
  await runQuery(
    ...
    (title, company_name, contact_name, contact_email, contact_phone, location, category, description, status)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [title, company_name, contact_name, contact_email, contact_phone, location, category, description, status || 'open']
  );
...
app.post('/manage/opportunity/:id/edit', requireRole('admin', 'employer'), async (req, res) => {
  ...
  await runQuery(
    `UPDATE opportunities
     SET title = ?, company_name = ?, contact_name = ?, contact_email = ?, contact_phone = ?,
         location = ?, category = ?, description = ?, status = ?
     WHERE id = ?`,
    [title, company_name, contact_name, contact_email, contact_phone, location, category, description, status, req.params.id]
  );
...
app.post('/api/ai/chat', requireAuth, async (req, res) => {
  ...
  let response = 'AI assistant response unavailable.';
  if (!prompt) {
    response = 'Please write a question.';
  } else if (/skill|skills|cv/i.test(prompt)) {
    ...
    response = 'Current CRM focus should be to track pending applications first, move accepted projects to in-progress, and mark completed projects when delivered. This keeps the pipeline operational and measurable.';
  } else {
    response = `Use this assistant for job matching, skills summaries, CRM summaries, and message drafting. Latest opportunities: ${latest.map((x) => `${x.title} (${x.location})`).join(', ')}. Opportunities by category: ${stats.map((s) => `${s.category}:${s.total}`).join(', ')}.`;
  }

  await runQuery('INSERT INTO aimessages (user_role, prompt, response) VALUES (?, ?, ?)', [
...
app.get('/api/charts', requireRole('admin'), async (req, res) => {
  ...
  const byLocation = await getAll(
    'SELECT location AS label, COUNT(*) AS value FROM opportunities GROUP BY location ORDER BY value DESC LIMIT 10'
  );
